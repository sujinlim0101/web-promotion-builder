  ...
  return (
    <form
      ...
      onChange={handleSubmit((data) =>
        updateAction(data, selectedComponent?.id)
      )}
    >
      <Select
        label="액션 타입"
      ...
      {watchActionType === "bridge" && (
        <div>
          ...
          {fields.map((field, index) => {
            return (
              <div key={field.id}>
                <Input name={`action.payload.${index}`} register={register} />
                <button
                  type="button"
                  className="flex text-sm ml-auto text-gray500"
                  onClick={() => {
                    remove(index);
